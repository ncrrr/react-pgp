...
class Crypt extends Component {
    ...
    crypt = async (e) => {
        e.preventDefault();
        const {
            text, key, signingOption, signingKey, passphrase,
        } = this.state;
    ...
    render() {
        const {
            text, key, signingOption, signingKey, passphrase, result, copied,
        } = this.state;
        return (
            <div id="crypt">
                        ...
                        <div className="row">
                            <div className="col-sm-12 col-md-6">
                                <div className="form-group">
                                    <label className="form-label" htmlFor="textCrypt">Text to crypt</label>
                                    <textarea name="textCrypt" id="textCrypt" rows="10" className="form-control" value={text} onChange={(e) => this.setState({ text: e.target.value })} required />
                              </div>
                          </div>
                            <div className="col-sm-12 col-md-6">
                                <div className="form-group">
                                    <label className="form-label" htmlFor="keyCrypt">Public key</label>
                                    <textarea name="keyCrypt" id="keyCrypt" rows="10" className="form-control" value={key} onChange={(e) => this.setState({ key: e.target.value })} required />
                              </div>
                          </div>
                      </div>
